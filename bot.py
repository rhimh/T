...
import threading, config, time, random, importlib
import redis
...
if R.get("{}:Nbot:restart".format(BOT_ID)):
  Bot("sendMessage",{"chat_id":R.get("{}:Nbot:restart".format(BOT_ID)),"text":"تم اعادة تشغيل البوت - Done restart the bot\nبواسطة: 𝗔𝗕𝗗𝗢 𝗔𝗦𝗜𝗟 - ســـــــــيزر","parse_mode":"html"})
...
def updateAuto(client, message,redis):
    ...
    if group is False and redis.get("{}Nbot:autoaddbot".format(BOT_ID)):
        ...
        if not redis.sismember("{}Nbot:disabledgroups".format(BOT_ID),chatID):
            ...
            kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/FA9SH")]])
            ...
            get = (redis.hget("{}Nbot:links".format(BOT_ID),chatID) or GetLink(chatID) or "https://t.me/FA9SH")
